# Flask #1: Применение WTForms для работы с формами сайта
...
# мы создавали формы вручную, прописывая их целиком в шаблонах, и если так делать, то за веб-программистом остаются задачи:
...
# WTForms – это библиотека способна генерировать формы, проверять их, наполнять начальной информацией, работать с reCaptcha и многое другое
# CSRF (Cross-Site Request Forgery) – межсайтовая подделка запросов
# Общая концепция создания форм -  состоит в расширении базового класса FlaskForm
# все поля формы описываются переменными этого класса и ссылаются на соответствующие объекты,
...
# Это лишь часть классов. Полную документация -  https://wtforms.readthedocs.io/en/3.0.x/

# Например создадим в нашем проекте вспомогательный файл forms.py,
# в котором будем определять все классы форм и начнем с класса LoginForm, класса авторизации:
