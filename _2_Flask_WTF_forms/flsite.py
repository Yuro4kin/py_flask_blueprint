# Flask #2: Обработка ошибок во Flask-WTF
...
# https://wtforms.readthedocs.io/en/3.0.x/ - документация
# Отображение ошибок в шаблонах форм - синтакис: form.<переменная поля>.errors
# Обращаемся к переменной form, которая обращается за то или иное поле и у этой переменной есть такая коллекция errors
# Эта коллекция как раз содержит строки с ошибками, переходим в шаблон login.html для написания проверки email
